...
exports.handler = async function (event) {
  try {
    const { messages, phase } = JSON.parse(event.body || '{}');

    if (!messages || !phase) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Chat failed', detail: 'Missing messages or phase' }),
      };
    }
    ...
    if (phase === 'clarify') {
      return {
        ...
        body: JSON.stringify({ reply: rawText }),
      };
    }
    ...
    try {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ reply: data.voice_summary, data }),
      };
    } catch (parseErr) {
      console.error('[chat] JSON parse failed:', rawText);
      return {
        statusCode: 500,
        body: JSON.stringify({ error: 'Chat failed', detail: 'Invalid JSON from Gemini' }),
      };
    }
  } catch (err) {
